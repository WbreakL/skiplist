...
private:
    ...
    int skip_list_level; // 当前跳表的 level

    Node <K, V>* header; // 指向头节点的指针

    int element_count;  // 跳表的当前 节点个数
...
SkipList<K, V>::~SkipList() {
    ...
    Node<K, V>* pNode = header->forward[0];
    //assert(pNode);
    while (pNode != nullptr)
    {
        Node<K, V>* tmp = pNode;
...
bool SkipList<K, V>::delete_element(K key) {
    ...
    if (current != nullptr && current->getKey() == key)
    {
        for (int i = 0; i < skip_list_level; ++i)
        {
            ...
            {
                break; // 此时break，跳出循环。后续的肯定也都是空了，不用continue
            }
...
int SkipList<K, V>::insert_element(K key, V value) {
    ...
    Node<K, V>* update[max_level + 1];//暂存新节点的forward
    memset(update, 0, sizeof(Node<K, V>*) * (max_level + 1));//？max_level+1
    ...
    current = current->forward[0];//指向下一个
    ...
    if (current == nullptr || current->getKey() != key)//注意顺序，否则可能segment fault
    {
        // 随机确定该插入节点的高度值
        int radm_level = get_randowm_level();

        // 如果 随机的高度比当前跳表的高度还高，需要将高出的 update 数组元素设置为 header，即指向header
        if (radm_level > skip_list_level)
        {
            for (int i = skip_list_level + 1; i < radm_level + 1; ++i)
            { // 第一次的 Segmentation fault (core dumped)，扩展数组的大小越界了
                update[i] = header;
            }
        ...
        // 更新所有的 level
        for (int i = 0; i <= radm_level; ++i)//注意：只赋值到radm_level即可，再后面全是0了
        {
            insert_node->forward[i] = update[i]->forward[i];//插入节点指向后一个节点
            update[i]->forward[i] = insert_node;//前一个节点指向新插入的节点
        }
...
// 将数据库中的kv 数据存入到文件中,数据库也就是此时内存中的跳表
template<typename K, typename V>
...
void SkipList<K, V>::load_data_file() {
    ...
    std::string line;
    std::string* key = new std::string();//写死
...
// 
template<typename K, typename V>
void SkipList<K, V>::get_key_value_from_string(const std::string& str, std::string* key, std::string* value) {//后两个为传出参数
...
// 
template<typename K, typename V>
void SkipList<K, V>::get_key_value_from_string(const std::string& str, K* key, V* value) {//后两个为传出参数
    if (!is_valid_string(str))
        return;
